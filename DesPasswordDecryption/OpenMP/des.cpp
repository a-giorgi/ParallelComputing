...
string encrypt(string input, string key, bool decrypt) {
    //cout << "Binary Key is: " << hex2bin(key)<< endl;
    //cout << "Binary Plaintext is: "<< hex2bin(input) << endl;
    string output = binEncrypt( hex2bin(input), hex2bin(key), decrypt);
    return bin2hex(output);


}
//This works just with binary values, we use this to make bruteforce attacks
string binEncrypt(string bin, string key, bool decrypt){
    ...
    // Splitting
    left = bin.substr(0, 32);
    right = bin.substr(32, 32);
    ...
    // Final Permutation
    string output = permute(combine, final_perm, 64);
    return output;
...
void findPlaintext(string hexCiphertext, string key, int length){
    long possiblePlaintexts = (long)pow((double)numChars,(double)length);
    char password[8];
    for(int i = 0; i < possiblePlaintexts; i++){
        generatePassword(password,length,i);
        string str(password);
        string enrcyptedPassword = encrypt(string2hex(str), key, false);
        if(enrcyptedPassword==hexCiphertext){
            cout << "Ciphertext decrypted: "<< str << endl;
            break;
        }else{
